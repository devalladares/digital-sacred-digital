...
gui.add(params, 'rotationSpeed').min(-0.005).max(0.005).name('Rotation Speed')
gui.add(params, 'gap').min(0.01).max(5).onFinishChange(generateMandala).name('Gap')
gui.add(params, 'ringNumber').min(1).max(12).step(1).onFinishChange(generateMandala).name('Number of Rings')
gui.add(params, 'emptySpace').min(1).max(5).onFinishChange(generateMandala).name('Centre Space')
